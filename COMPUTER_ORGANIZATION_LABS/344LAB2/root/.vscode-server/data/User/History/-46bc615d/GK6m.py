...
while iteration < max_iterations:
    ...
    # Before flying, check if we should continue after landing and refueling
    src_idx = current_airport
    if not check_any_westward_reachable_after_refuel(next_idx, current_fuel - fuel_needed):
        print(f"After landing at {airport_names[next_idx]} and refueling, no westward route possible. Stopping here.")
        break
    
    # Execute flight
    if not execute_flight(current_airport, next_idx):
        print("Flight failed!")
        break
...
print(f"Flight path: {' -> '.join([str(i) for i in flight_history])}")
print(f"Actual:   {flight_history}")
print(f"Expected: {expected_history}")
print(f"Match: {flight_history == expected_history}")
print()
