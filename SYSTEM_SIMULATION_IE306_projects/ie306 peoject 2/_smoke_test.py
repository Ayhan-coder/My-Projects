...
# ===== Cell 13 =====
# ─── V&V: Downstream blocking stress test ──────────────────────────────────
# At the parameters given (lambda=900 veh/h, Nmax=40), average link occupancy
# is ~4.3 vehicles and blocking never fires.  To verify the blocking LOGIC is
# correct, we run a single short replication at 3x demand (2700 veh/h) with
# no emergency vehicles and confirm that blocking_frequency > 0.

import simpy as _simpy, math as _math

_HIGH_LAMBDA_S = 2700 / 3600.0   # 3x normal southbound demand

def _run_stress(lambda_s=_HIGH_LAMBDA_S, seed=42):
    import numpy as np
    ...
    # Custom high-demand south arrival process
    def _arrivals(env, inter, rng_arr, rng_cls, rng_turn):
        vid = 0
        while True:
            dt = rng_arr.exponential(1.0 / lambda_s)
            yield env.timeout(dt)
            if env.now > RUN_LENGTH:
                break
            kind = 'car' if rng_cls.random() < P_CAR else 'bus'
            veh = Vehicle(vid=vid, kind=kind, entry_time=env.now,
                          approach_arrival=env.now,
                          turn_e_at_A=(rng_turn.random() > P_STRAIGHT_A))
            vid += 1
            inter.enqueue_ns(veh)

    env.process(_arrivals(env, iA, rngs['arrival_s'], rngs['classify'], rngs['turn_A']))
...
_stress_row = _run_stress()
_bf  = _stress_row['blocking_frequency']
_occ = _stress_row['avg_link_occupancy']
print('=== Blocking Stress Test (lambda = 2700 veh/h, 3x normal) ===')
print(f'  avg_link_occupancy : {_occ:.2f}  (capacity = {N_MAX})')
print(f'  blocking_frequency : {_bf:.4f}')
if _bf > 0:
    print('  PASS: blocking logic fires correctly at elevated demand.')
else:
    print('  NOTE: blocking still did not fire — check link model or demand level.')
